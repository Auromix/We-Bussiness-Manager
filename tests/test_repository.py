...
from database.models import Employee, Customer, ServiceType, ServiceRecord, Product, ProductSale, Membership
...
class TestDatabaseManager:
    ...
    def test_get_or_create_customer(self, temp_db):
        """测试获取或创建顾客"""
        customer = temp_db.customers.get_or_create("段老师")
        assert customer.id > 0
        assert customer.name == "段老师"
        
        # 再次获取应该返回同一个
        customer2 = temp_db.customers.get_or_create("段老师")
        assert customer.id == customer2.id
    
    def test_get_or_create_employee(self, temp_db):
        """测试获取或创建员工"""
        employee = temp_db.staff.get_or_create("叶维忠", "六亿（叶维忠）")
    ...
    def test_get_or_create_service_type(self, temp_db):
        """测试获取或创建服务类型"""
        service_type = temp_db.service_types.get_or_create("头疗", 30.0, "therapy")
    ...
    def test_save_service_record(self, temp_db):
        ...
        # 先创建相关实体
        customer = temp_db.customers.get_or_create("段老师")
        service_type = temp_db.service_types.get_or_create("头疗", 30.0)
    ...
    def test_get_daily_records(self, temp_db):
        """测试按日期获取记录"""
        # 创建几条记录
